...
public class FileIndexBuilder {
	...
	public static void main(String[] args) throws Exception {
		...
		b.addFiles(FileFinder.GetAllFiles("src/search/email/documents", null, true),
				true /*clear_old_index = false if adding*/);
		for (int x = 0 ; x <100 ;x++){
			if (x<10){
				b.addFiles(FileFinder.GetAllFiles("src/search/government/documents/0"+x+"/", null, true),
						true /*clear_old_index = false if adding*/);
			}else{
				b.addFiles(FileFinder.GetAllFiles("src/search/government/documents/"+x+"/", null, true),
						true /*clear_old_index = false if adding*/);
			}
		}
